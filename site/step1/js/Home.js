...
class Home extends React.Component {
	...
	render() {
		return React.createElement(
			"form",
			null,
			React.createElement("input", { value: this.state.text, onChange: this.handleChange, placeholder: "what's on you'r mind...", type: "text", id: "text" }),
			React.createElement(
				"button",
				{ type: "button", onClick: this.new_post },
				"post"
			),
			React.createElement(
				"div",
				{ className: "Posts_refresher" },
				React.createElement(
					"button",
					{ type: "button", onClick: this.refresh_posts, style: this.state.buttonStyle },
			...
			React.createElement(
				...
				this.state.posts.slice(0, 3).map((item, index) => {
					return React.createElement(Post, { "class": "form", post: item, key: index });
				})
			)
		);
	}
