...
class Messages extends React.Component {
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement('textarea', { 'class': 'messageTextInput', value: this.state.text, onChange: this.handleChange, placeholder: 'message', type: 'text', id: 'text' }),
                    React.createElement(
                        'div',
                        { 'class': 'sendNewMessageButtons' },
                        React.createElement('input', { type: 'number', id: 'inputId', min: '1', placeholder: 'Recipient\'s user ID' }),
                        React.createElement(
                            'button',
                            { type: 'button', onClick: this.new_message },
                            'Send Message'
                        ),
                        React.createElement(
                            'button',
                            { type: 'button', onClick: this.refresh_messages, style: this.state.buttonStyle },
    ...
    map_messages() {
        ...
        const mappedMessages = this.state.messages.slice(0, amoutOfMessages).map((message, index) => {
            return React.createElement(Message, { 'class': 'form', message: message, key: index });
        });

        if (mappedMessages.length == 0) {
            const msg = { text: "No Messages to display" };
            const emptyMsg = React.createElement(Message, { 'class': 'form', message: msg, key: 0 });
